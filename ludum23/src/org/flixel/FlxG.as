package org.flixel
{
	...
	public class FlxG
	{
		...
		/**
		 * Box2D pixels per meter unit
		 */
		static public const B2SCALE:Number = 16;
		...
		/**
		 * Gets distance between 2 FlxB2Sprites
		 */
		public static function getDistance(A:FlxB2Sprite, B:FlxB2Sprite):FlxPoint
		{
			var inp:b2DistanceInput = new b2DistanceInput();
			var out:b2DistanceOutput = new b2DistanceOutput();
			var ca:b2SimplexCache = new b2SimplexCache();
			
			inp.proxyA = new b2DistanceProxy();
			inp.proxyA.Set(A.fixtureDef.shape);
			inp.proxyB = new b2DistanceProxy();
			inp.proxyB.Set(B.fixtureDef.shape);
			
			inp.transformA = A.body.GetTransform();
			inp.transformB = B.body.GetTransform();
			ca.count = 0;
			b2Distance.Distance(out, ca, inp);
			
			var retv:b2Vec2 = out.pointB;
			retv.Subtract(out.pointA);
			retv.Multiply(B2SCALE);
			
			return new FlxPoint(retv.x, retv.y);
		}
		...
		static public function flash(Color:uint=0xffffffff, Duration:Number=1, OnComplete:Function=null, Force:Boolean=false):void
		{
			...
			*/
			var l:uint = FlxG.cameras.length;
			if(l)
				(FlxG.cameras[l-1] as FlxCamera).flash(Color,Duration,OnComplete,Force);
		}
		...
		static public function fade(Color:uint=0xff000000, Duration:Number=1, OnComplete:Function=null, Force:Boolean=false):void
		{
			...
			*/
			var l:uint = FlxG.cameras.length;
			if(l)
				(FlxG.cameras[l-1] as FlxCamera).fade(Color,Duration,OnComplete,Force);
		}
		...
		static public function shake(Intensity:Number=0.05, Duration:Number=0.5, OnComplete:Function=null, Force:Boolean=true, Direction:uint=0):void
		{
			...
			*/
			var l:uint = FlxG.cameras.length;
			if(l)
				(FlxG.cameras[l-1] as FlxCamera).shake(Intensity, Duration, OnComplete, Force, Direction);
		}
